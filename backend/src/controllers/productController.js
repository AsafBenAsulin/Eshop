...
const getProductByQuery = async (req, res) => {
  ...
  const products = await Product.find(
    { ...queryFilter, ...categoryFilter, ...priceFilter, ...ratingFilter }).sort(
      sortOrderFilter).skip(
        pageSize * (page - 1)).limit(pageSize);
...
export { productData, getProductById, getProductByToken, getCategories, getProductByQuery }